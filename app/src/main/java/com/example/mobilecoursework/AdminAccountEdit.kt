...
class AdminAccountEdit : AppCompatActivity() {
    ...
    fun saveButton(view: View) {
        ...
        } else if (existingUsers!!.moveToFirst() && from == "accountEdit" && existingUsers.getInt(0)
                .toString() != accountId
        ) {
            errorMessageUserName = "username all ready taken"
        }
        ...
        if (!(findViewById<RadioButton>(R.id.rbYesAdminAccountEdit).isChecked) && !(findViewById<RadioButton>(
                R.id.rbNoAdminAccountEdit
            ).isChecked)
        ) {
